...
export function Visual1({
  ...
  gridColor = "#80808015",
  title = "Random Data Visualization",
  description = "Displaying some interesting stats.",
  label1 = "Tommy",
  label2 = "Megan",
  id,
}: Visual1Props) {
  return (
    ...
    >
      ...
      <Layer2 color={mainColor} id={id} />
      <Layer3 color={mainColor} secondaryColor={secondaryColor} label1={label1} label2={label2} />
      <Layer4 title={title} description={description} />
      <EllipseGradient color={mainColor} id={id} />
...
const EllipseGradient = ({ color, id = "paint" }: EllipseGradientProps) => {
  ...
  return (
    <div className="absolute inset-0 z-[5] flex h-full w-full items-center justify-center">
      ...
      >
        <rect width="356" height="180" fill={`url(#${uniqueId})`} />
        <defs>
          <radialGradient
            id={uniqueId}
            cx="0"
...
interface Layer2Props extends LayerProps {
  id?: string;
}

const Layer2 = ({ color, id }: Layer2Props) => {
  const clipId = `clip-${id || Math.random().toString(36).substr(2, 9)}`;
  return (
...
const Layer3 = ({ color, secondaryColor, label1 = "Tommy", label2 = "Megan" }: Layer3Props) => {
  return (
    <div
...
const Layer4 = ({ title = "Random Data Visualization", description = "Displaying some interesting stats." }: Layer4Props) => {
  return (
